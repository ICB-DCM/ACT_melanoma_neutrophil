...
# revision: Updated with marker for mouse sex and max_iterations
# '''=================================================
import sys
...
females_rm = False
if females_rm:
    modifier = f'{img_modifier}_{metacluster_key}_fp_{force_parametric}_fem_removed'
else:
...
# if females_rm, remove entries with mouse_id in female_mice_ids from df_t_zone_proximity
print(len(df_t_zone_proximity))
if females_rm:
    df_t_zone_proximity = df_t_zone_proximity[~df_t_zone_proximity['mouse_id'].isin(female_mice_ids)
    ].copy()
print(len(df_t_zone_proximity))
...
plt.savefig(f'{fig_dir}/Neutrophil_ppa_{condition}_{value_col}_ratio_paired_{modifier}_sex.pdf')
#%%
condition = 'ACT_d14'
...
ax.set_xlabel('')
plt.savefig(f'{fig_dir}/Neutrophil_ppa_{condition}_ratio_paired_{modifier}_sex.pdf')
