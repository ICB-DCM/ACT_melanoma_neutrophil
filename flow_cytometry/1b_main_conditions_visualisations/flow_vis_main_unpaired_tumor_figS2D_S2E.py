...
# @Desc updated: Unpaired CD8 panel tumor plots for main conditions.
# update 20260210: on main conditions, generates plots for fig S2D and S2E
# '''=================================================
import os
import sys
...
#%%
lymph_nodes = ['Tumour']
colours_conditions = bar_colours_hex_blood_flow[3:]
x_axis_tick_labels = x_axis_tick_labels_flow[3:]  # only the ACT conditions
conditions = ['ACT-d3', 'ACT-d7', 'ACT-d14', 'ACT_Relapse']
conditions_to_compare = [(conditions[i], conditions[i + 1]) for i in
...
stats_df_cd8.to_csv(f'{stats_dir}/ln_cd8_stats_CpG_perturbations_{test_function_used}_mtc{mtc}_'
                    f'np_{non_parametric}_fp_{force_parametric}_{modifier}_tumor.csv')
#%% plotting
...
for cell_type_i, cell_type in enumerate(value_cols):
    ...
    for i_LN, LN in enumerate(lymph_nodes):
        ...
        boxplot_unpaired(data, group_col, cell_type, stats_df=stats_df_LN, ax=ax, p_val_col_name=p_val_col_name,
                         ...
                         significance_bar_order=['low', 'high', 'low', 'high'])
        # ax.set_title(f'{LN}', y=1.05)
        ax.set_title('')  # remove title
        ax.set_ylabel(f'{value_cols_y_label[cell_type_i]}')
        ax.set_xlabel("")
        ax.set_xticklabels(x_axis_tick_labels)  #, rotation=90)
    # plt.tight_layout()
    plt.savefig(f'{fig_path}/unpaired/{cell_type}_flow_boxplot_CpG_perturbations_{test_used}_'
