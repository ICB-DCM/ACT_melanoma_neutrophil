...
value_cols_y_label = ['pmel-1 $T_{EFF}$ \nFrequency of pmel-1 cells',
                      'pmel-1 $T_{CM}$ \nFrequency of pmel-1 cells']
img_modifier='incl_102_d14_pmelfreq_only'  # incl_102_d14
...
for cell_type_i, cell_type_single in enumerate(cell_types_single):
    ...
    plt.savefig(f'{fig_path}/single_paired/{cell_type_single}_{condition}_ratio_paired_counts_{img_modifier}.pdf')
# %% save mouse_ID value counts
#save to file
df_complete['Mouse_ID'].value_counts().sort_index().to_csv(f'{processed_data_path}/mouse_id_counts_complete.csv')
